...
`
  ...
  try {
    ...
    assert.match(markdown, /Edit only the supplied working files/);
    assert.match(markdown, /wrapper owns branch selection, commits, pushes, and pull request creation/);
    assert.doesNotMatch(markdown, /gh pr|git push|git commit/);
